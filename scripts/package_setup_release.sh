...
#
# Writes: dist/colony-wars-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix colony-wars \
  --exe-name Colony_Wars_Recompiled \
  --display-name "Colony Wars Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Colony Wars (USA) (Disc 1).cue" \
